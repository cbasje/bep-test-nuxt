...
const Stats = __webpack_require__(747).Stats
...
function redirect(res, statusOrUrl, url) {
    ...
    if (typeof statusOrUrl !== 'number' || typeof url !== 'string') {
        throw new TypeError(`Invalid redirect arguments. Please use a single argument URL, e.g. res.redirect('/destination') or use a status code and URL, e.g. res.redirect(307, '/destination').`);
    }
...
function send(req, res, body) {
    ...
    if (chunk !== undefined) {
        if (Buffer.isBuffer(chunk)) {
            ...
            throw new TypeError('`body` is not a valid string, object, boolean, number, Stream, or Buffer');
        }
        if (len !== undefined) {
